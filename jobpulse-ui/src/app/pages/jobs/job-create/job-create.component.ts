      ...
      <form (ngSubmit)="onSubmit()" class="form-container">
        <div class="form-section">
          ...
          <div class="form-group">
            <label for="payload">
              Payload
              <button type="button" class="reset-btn" (click)="resetPayload()">
                Reset template
              </button>
            </label>
            <textarea
          ...
          @if (job.schedule.type === 'RECURRING') {
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <select
                  id="frequency"
                  [(ngModel)]="job.schedule.frequency"
                  name="frequency"
                  required
                >
          ...
          @if (job.schedule.type === 'CRON') {
            <div class="form-group">
              ...
              <input
                id="cronExpression"
                type="text"
                [(ngModel)]="job.schedule.cronExpression"
                name="cronExpression"
...
export class JobCreateComponent implements OnInit {
  ...
  private readonly typeDescriptions: Record<string, string> = {
    ...
    [JobType.REPORT_GENERATION]:
      'Generates a report: user_activity, job_execution_stats, or system_health',
  };
  ...
  private readonly payloadHints: Record<string, string> = {
    ...
    [JobType.DATA_CLEANUP]:
      'Required: action (delete_old_records | truncate_table | archive_data) + action-specific fields',
    [JobType.REPORT_GENERATION]:
      'Required: reportType (user_activity | job_execution_stats | system_health)  •  Optional: outputFormat',
  };
  ...
  onSubmit(): void {
    ...
    this.jobService.createFull(this.job).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        ...
        const msg = err.error?.message || err.error || 'Failed to create job.';
