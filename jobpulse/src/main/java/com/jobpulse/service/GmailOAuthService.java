...
/**
 * Handles the Gmail OAuth2 flow: 1. Build the Google consent URL (with offline access → gives us a
 * refresh token) 2. Exchange the authorization code for access + refresh tokens 3. Store tokens
 * encrypted in the database 4. Refresh an expired access token transparently 5. Provide a decrypted
...
public class GmailOAuthService {
  ...
  @Autowired private GmailTokenRepository gmailTokenRepository;
  ...
  /**
   * Stores Gmail tokens from an already-exchanged token response. Use this when the code has
   * already been exchanged elsewhere (e.g., during Google OAuth login).
   */
  @Transactional
  public GmailTokenInfo storeTokensFromResponse(User user, JsonNode tokenResponse) {
    ...
    GmailToken token =
        gmailTokenRepository.findByUser(user).orElse(GmailToken.builder().user(user).build());
  ...
  private JsonNode exchangeCode(String code) {
    try {
      ...
      if (json.has("error")) {
        throw new BadRequestException(
            "Google token exchange failed: " + json.get("error").asText());
      }
      ...
      throw new BadRequestException(
          "Failed to exchange Google authorization code: " + e.getMessage());
    }
  }
