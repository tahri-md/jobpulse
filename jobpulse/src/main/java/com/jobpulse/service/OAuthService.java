...
public class OAuthService {
  ...
  @SuppressFBWarnings(
      value = "EI_EXPOSE_REP",
      justification = "DTO wrapper intentionally exposes User")
  ...
  public User processGitHubOAuth(String code) {
    log.info(
        "Processing GitHub OAuth with code: {}...", code.substring(0, Math.min(10, code.length())));
  ...
  private User getOrCreateUser(OAuthUserInfo userInfo, AuthProvider provider) {
    ...
    User user =
        userRepository
            .findByIdentities_ProviderAndIdentities_ProviderId(provider, userInfo.getProviderId())
            .orElse(null);
    ...
    if (user != null) {
      ...
      UserIdentity newIdentity =
          UserIdentity.builder().provider(provider).providerId(userInfo.getProviderId()).build();
